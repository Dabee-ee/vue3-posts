  ...
  <div>
    ...
    <div class="row g-2">
      <div v-if="item?.prevId" class="col-auto">
        <button class="btn btn-outline-dark" @click="goDetailPage(item.prevId)">이전글</button>
      </div>
      <div v-if="item?.nextId" class="col-auto">
        <button class="btn btn-outline-dark" @click="goDetailPage(item.nextId)">다음글</button>
      </div>
